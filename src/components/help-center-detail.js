...
import {
    countHelpArticleByCategory,
    helpArticlesListByCategory,
} from "../redux/actions/register/help";

class HelpCenterDetails extends Component{
    constructor(props) {
        super(props);
    ...
    componentDidMount() {
        ...
        if(countHelpArticleByCategory) {
            countHelpArticleByCategory(data);
        }
        ...
        if(helpArticlesListByCategory) {
            helpArticlesListByCategory(datas);
        }
        window.scrollTo(0, 0);
    }

    componentDidUpdate(prevProps, prevState, snapshot) {
        if(this.props.countArticleByCategory && prevProps.countArticleByCategory !== this.props.countArticleByCategory) {
            this.setState({
                articleCountInfo: this.props.countArticleByCategory,
            })
        }

        if(this.props.helpArticlesByCategory && prevProps.helpArticlesByCategory !== this.props.helpArticlesByCategory) {
            this.setState({
                articleList: this.props.helpArticlesByCategory.list,
                page_num: this.props.helpArticlesByCategory.page_num,
            })
        }
    }
    ...
    onPageClick = (item) => {
        ...
        if(helpArticlesListByCategory) {
            helpArticlesListByCategory(data)
        }
        window.scrollTo(0, 0);
    };
    ...
    render() {
        ...
        if( this.props.helpArticlesByCategory ) console.log("articleList = ", this.props.helpArticlesByCategory);
            ...
            <div className="help-article-bgColor">
                ...
                <div className="whole-body-mW help-article-pt align-l">
                    <div>
                        ...
                        <span className="txt-16 col-heavyDark mouse-cursor">{this.state.articleCountInfo[0] && this.state.articleCountInfo[0].category}</span>
                    </div>

                    {
                        this.state.articleCountInfo[0] && (
                            <>
                                <div className="txt-30 col-txt-title general-articles general-pt">{this.state.articleCountInfo[0].category}</div>
                                <div className="general-flex">
                                    <img src={require('../assets/img/help-center-icon-' + this.state.articleCountInfo[0].key + '.svg')} alt="" />
                                    <span className="help-article-p6">
                                    <div className="txt-16 col-desc">
                                        {this.state.articleCountInfo[0].description}
                                    </div>
                                    </span>
                                </div>

                                <div className="txt-16 col-blue general-articles mouse-cursor">
                                    {this.state.articleCountInfo[0].nCount? this.state.articleCountInfo[0].nCount: 0}
                                    {this.state.articleCountInfo[0].nCount > 1? " articles" : " article"}
                                </div>
                            </>
                        )
                    }
                    ...
                    {
                        this.state.articleList && this.state.articleList.map((item, key) => {
                            return (
...
const mapStateToProps = (state) => {
    return {
        countArticleByCategory: state.registers.countArticleByCategory,
        helpArticlesByCategory: state.registers.helpArticlesByCategory,
        msg_articleByTitle: state.registers.msg_articleByTitle,
    }
};
